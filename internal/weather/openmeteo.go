...
// FetchReport queries the Open-Meteo API and returns a 5-slot WeatherReport.
// Pure HTTP + parse — no DB, no env reads.
func FetchReport(lat, lon string) (*WeatherReport, error) {
	if lat == "" || lon == "" {
		return nil, fmt.Errorf("LAT/LON not set")
	}
	resp, err := http.Get(BuildURL(lat, lon))
	if err != nil {
...
// WMOCodeText converts a WMO weather interpretation code to a human-readable string.
